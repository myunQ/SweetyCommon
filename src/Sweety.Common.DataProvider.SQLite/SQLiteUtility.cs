...
namespace Sweety.Common.DataProvider.SQLite
{
    ...
    public class SQLiteUtility : RelationalDBUtilityBase
    {
...
#endif
            {
                throw new InvalidCastException(String.Format(Common.Properties.Localization.is_not_a_valid_object_of_type_XXX, nameof(command), typeof(SQLiteCommand).FullName));
            }
...
#endif //NETSTANDARD2_0
        {
            if (!(conn is SQLiteConnection result)) throw new ArgumentException(String.Format(Common.Properties.Localization.this_instance_only_accepts_database_connection_objects_of_type_XXX, typeof(SQLiteConnection).FullName), nameof(conn));
...
#endif // NETSTANDARD2_0
        {
            if (!(tran is SQLiteTransaction result)) throw new ArgumentException(String.Format(Common.Properties.Localization.this_instance_only_accepts_transaction_objects_of_type_XXX, typeof(SQLiteTransaction).FullName), nameof(tran));
        ...
        #region SqlHelper
        /// <summary>
        /// 将<paramref name="parameters"/>中的元素添加到<see cref="SQLiteCommand.Parameters"/>集合。
...
#if NETSTANDARD2_0
        private void AttachParameters(SQLiteCommand command, IEnumerable<SQLiteParameter> parameters)
        {
        ...
        /// <exception cref="ArgumentNullException">当<paramref name="command"/>为<c>null</c>是引发此异常。</exception>
#if NETSTANDARD2_0
        private void AttachParameters(SQLiteCommand command, IEnumerable<IDataParameter> parameters)
        {
            if (parameters is IEnumerable<SQLiteParameter> sqlParameters)
            {
...
#if NETSTANDARD2_0
        private void PrepareCommand(SQLiteCommand command, CommandType commandType, IEnumerable<IDataParameter> commandParameters, out bool mustCloseConnection)
#else
        private void PrepareCommand(SQLiteCommand command, CommandType commandType, IEnumerable<IDataParameter?>? commandParameters, out bool mustCloseConnection)
#endif // NETSTANDARD2_0
        {
...
#if NETSTANDARD2_0
        private async ValueTask<bool> PrepareCommandAsync(SQLiteCommand command, CommandType commandType, IEnumerable<IDataParameter> commandParameters, CancellationToken cancellationToken = default)
#else
        private async ValueTask<bool> PrepareCommandAsync(SQLiteCommand command, CommandType commandType, IEnumerable<IDataParameter?>? commandParameters, CancellationToken cancellationToken = default)
#endif
        {
        ...
        /// <returns>返回受影响的记录数。</returns>
#if NETSTANDARD2_0
        private T ExecuteSql<T>(Func<SQLiteCommand, T> exec, IDbTransaction transaction, string commandText, CommandType commandType, IEnumerable<IDataParameter> commandParameters)
#else
        private T ExecuteSql<T>(Func<SQLiteCommand, T> exec, IDbTransaction transaction, string commandText, CommandType commandType, IEnumerable<IDataParameter?>? commandParameters)
#endif
        {
            ...
            SQLiteCommand cmd = new SQLiteCommand(commandText, tran.Connection, tran);
            try
            {
                // SQL Server 要获取 SqlTransaction 对象实例的话连接必须是打开的，所以如果传入 transaction 就可以忽略 connection 参数了。
        ...
        /// <returns>返回受影响的记录数。</returns>
#if NETSTANDARD2_0
        private T ExecuteSql<T>(Func<SQLiteCommand, T> exec, IDbConnection connection, string commandText, CommandType commandType, IEnumerable<IDataParameter> commandParameters)
#else
        private T ExecuteSql<T>(Func<SQLiteCommand, T> exec, IDbConnection connection, string commandText, CommandType commandType, IEnumerable<IDataParameter?>? commandParameters)
#endif
        {
            ...
            SQLiteCommand cmd = new SQLiteCommand(commandText, ConvertToSqlConnection(connection));
            try
            {
        ...
        /// <returns>返回受影响的记录数。</returns>
#if NETSTANDARD2_0
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CancellationToken, Task<T>> execAsync, IDbTransaction transaction, string commandText, CommandType commandType, CancellationToken cancellationToken = default, IEnumerable<IDataParameter> commandParameters = null)
#else
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CancellationToken, Task<T>> execAsync, IDbTransaction transaction, string commandText, CommandType commandType, CancellationToken cancellationToken = default, IEnumerable<IDataParameter?>? commandParameters = null)
#endif
        {
        ...
        /// <returns>返回受影响的记录数。</returns>
#if NETSTANDARD2_0
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CancellationToken, Task<T>> execAsync, IDbConnection connection, string commandText, CommandType commandType, CancellationToken cancellationToken = default, IEnumerable<IDataParameter> commandParameters = null)
#else
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CancellationToken, Task<T>> execAsync, IDbConnection connection, string commandText, CommandType commandType, CancellationToken cancellationToken = default, IEnumerable<IDataParameter?>? commandParameters = null)
#endif
        {
        ...
        /// <returns>返回包含结果集的数据读取器</returns>
#if NETSTANDARD2_0
        private T ExecuteSql<T>(Func<SQLiteCommand, CommandBehavior, T> exec, IDbTransaction transaction, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter> commandParameters)
#else
        private T ExecuteSql<T>(Func<SQLiteCommand, CommandBehavior, T> exec, IDbTransaction transaction, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter?>? commandParameters)
#endif
        {
        ...
        /// <returns>返回包含结果集的数据读取器</returns>
#if NETSTANDARD2_0
        private T ExecuteSql<T>(Func<SQLiteCommand, CommandBehavior, T> exec, IDbConnection connection, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter> commandParameters)
#else
        private T ExecuteSql<T>(Func<SQLiteCommand, CommandBehavior, T> exec, IDbConnection connection, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter?>? commandParameters)
#endif
        {
        ...
        /// <returns>返回包含结果集的数据读取器。</returns>
#if NETSTANDARD2_0
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CommandBehavior, CancellationToken, Task<T>> execAsync, IDbTransaction transaction, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter> commandParameters, CancellationToken cancellationToken = default)
#else
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CommandBehavior, CancellationToken, Task<T>> execAsync, IDbTransaction transaction, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter?>? commandParameters, CancellationToken cancellationToken = default)
#endif
        {
        ...
        /// <returns>返回包含结果集的数据读取器。</returns>
#if NETSTANDARD2_0
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CommandBehavior, CancellationToken, Task<T>> execAsync, IDbConnection connection, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter> commandParameters, CancellationToken cancellationToken = default)
#else
        private async Task<T> ExecuteSqlAsync<T>(Func<SQLiteCommand, CommandBehavior, CancellationToken, Task<T>> execAsync, IDbConnection connection, string commandText, CommandType commandType, CommandBehavior commandBehavior, IEnumerable<IDataParameter?>? commandParameters, CancellationToken cancellationToken = default)
#endif
        {
        ...
        /// <returns></returns>
        private Task<int> ExecuteNonQueryAsync(SQLiteCommand cmd, CancellationToken cancellationToken)
        ...
        /// <returns></returns>
#if NETSTANDARD2_0
        private Task<object> ExecuteScalarAsync(SQLiteCommand cmd, CancellationToken cancellationToken)
#else
        private Task<object?> ExecuteScalarAsync(SQLiteCommand cmd, CancellationToken cancellationToken)
#endif
            => cmd.ExecuteScalarAsync(cancellationToken);

        private IDataReader ExecuteReader(SQLiteCommand cmd, CommandBehavior commandBehavior)
            => cmd.ExecuteReader(commandBehavior);

        private (IDataReader, IDbCommand) ExecuteReaderAndReturnCommand(SQLiteCommand cmd, CommandBehavior commandBehavior)
            => (cmd.ExecuteReader(commandBehavior), cmd);

        private async Task<IDataReader> ExecuteReaderAsync(SQLiteCommand cmd, CommandBehavior commandBehavior, CancellationToken cancellationToken)
            => await cmd.ExecuteReaderAsync(commandBehavior, cancellationToken);

        private async Task<(IDataReader, IDbCommand)> ExecuteReaderAndReturnCommandAsync(SQLiteCommand cmd, CommandBehavior commandBehavior, CancellationToken cancellationToken)
